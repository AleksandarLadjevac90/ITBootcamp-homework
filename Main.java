...
public class Main {
	...
	public static void main(String[] args) {
	    Album love_supreme = new Album("A Love Supreme", "John Coltrane", 1965);
	    ...
	    Album animals = new Album("Animals", "Pink Floyd", 1977);
	...
	public static void simulacija(Prodavnica p, String zelja) {
	    ...
	    if(ponuda.length==0) {
	        ...
	        boolean kupio = false;
	        int kolicina = 0;
	        while(ponuda.length>0 && kupio==false && ponuda[kolicina]!=null) {
	            System.out.println("Kupac: Kupiću -- " + ponuda[kolicina].ime());
	            kupio = ponuda[kolicina].kupi();
	            if(!kupio) {
	                System.out.print("Prodavac: ");
	                System.err.println("Nažalost nemamo traženi artikal trenutno :(\n");
	                ponuda[kolicina] = null;
	            }
	            kolicina++;
	        }
	        if(kupio) {
